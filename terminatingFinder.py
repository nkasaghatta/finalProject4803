...
def gibbs(sequences, seqCount, seqLength, ML, iterations):
    ...
    for run in range(0, iterations):
        for n in range(seqCount):
            ...
            startingIndexes[n] = nextStart

        currentICPC = calcICPC(motifPWM(motifs, ML), bgFreq)
        if len(prevPWM) == 0:
            outputPWM = motifPWM(motifs, ML)
            prevPWM = outputPWM
            prevStartingIndexes = startingIndexes
            lastICPC = currentICPC
        elif currentICPC > calcICPC(prevPWM, bgFreq):
            prevPWM = motifPWM(motifs, ML)
            prevStartingIndexes = startingIndexes
            lastICPC = currentICPC
            lastChangeIteration = run
        elif currentICPC < calcICPC(prevPWM, bgFreq):
            outputPWM = prevPWM
            startingIndexes = prevStartingIndexes
        ICPCdata.append(lastICPC)
        print('RUN: ' + str(run) + '       ICPC: ' + str(ICPCdata[run]))
        if currentICPC >= 1.97 and (run - lastChangeIteration) > 1000:
            print("Early termination: ICPC has stabilized above 1.97 and unchanged for more than 1000 iterations.")
            break
